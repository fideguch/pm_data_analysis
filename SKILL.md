...
- `templates/analysis_report.md` → Load only when generating final report
...
2. **Exists:** Load silently. Check freshness based on business stage:
   - Pre-PMF: warn if > 7 days old
   - Growth: warn if > 14 days old
   - Scale/Mature: warn if > 30 days old
   - Load `analysis/history/` — latest 3 reports full, older ones via 1-line summary in context.md only
3. **Missing:** Run initial hearing (required 3 items only), then generate context.md from `templates/context.md`
...
For every analysis plan, define guardrail metrics. If user hasn't specified, suggest defaults:

```
Default Guardrail Metrics by Business Model:
| Model | Guardrail 1 | Guardrail 2 |
|-------|------------|------------|
...
- [ ] Instrumentation Bias: Was there a recent logging/tracking/SDK change?
- [ ] Seasonality: Are we comparing equivalent time periods (e.g., weekday vs weekday)?
```
...
Follow global CLAUDE.md language settings. Section headers and statistical notation in English.
